...
class RuningScore(object):
    ...
    def get_scores(self):
        ...
        acc = np.diag(hist).sum() / (hist.sum() + 0.000001)
        ...
        freq = hist.sum(axis=1) / (hist.sum() + 0.000001)
...
class TextScores(RuningScore):
    def __init__(self, n_classes, thresh=0.5):
        super().__init__(n_classes=n_classes)
        self.thresh = thresh
    
    def update(self, preds, gts, masks):
        masks = masks.numpy()
        pred_text = preds.numpy() * masks
        pred_text = np.where(pred_text > self.thresh, 1, 0).astype(np.int32)
        gt_text = gts.numpy() * masks
        gt_text = gt_text.astype(np.int32)
        super().update(gt_text, pred_text)

    @property
    def scores(self):
        score_text, _ = self.get_scores()
