...
class RandomCropData(object):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # crop 图片 保持比例填充
        scale_w = self.size[1] / crop_w
        scale_h = self.size[0] / crop_h
        scale = min(scale_w, scale_h)
        h = int(crop_h * scale)
        w = int(crop_w * scale)
        if self.keep_ratio:
            if len(im.shape) == 3:
                padimg = np.zeros((self.size[0], self.size[1], im.shape[2]), im.dtype)
            else:
                padimg = np.zeros((self.size[0], self.size[1]), im.dtype)
        ...
        data['img'] = img
        data['text_polys'] = np.array(text_polys_crop, dtype=np.float32)
